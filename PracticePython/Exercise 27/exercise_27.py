...
# about a tic tac toe game. In a tic tac toe game, the “game server” needs to know where the Xs and Os are in the board,
...
def make_a_move(board, player):
    # print(f"plansza wygląda następująco:")
    # for i in board:
    #     print(i)
    x_move = input(f"Graczu {player}, podaj lokalizacje punktu w którym chcesz postawić {player}, w formacie 'rząd, kolumna'(np '3,3'): ")

    while len(x_move) != 3 or x_move[0] not in ["1", "2", "3"] or x_move[2] not in ["1", "2", "3"] or x_move[1] != ",":
        x_move = input("POPRAWNY FORMAT to 'rząd*,kolumna*' *(od 1 do 3): ")

    if board[int(x_move[0])-1][int(x_move[2])-1] == 0:
        board[int(x_move[0])-1][int(x_move[2])-1] = player
    else:
